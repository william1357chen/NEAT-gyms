...
class MountainCar:
    ...
    def eval_genome(genome, config):
        ...
        for run in range(runs_per_net):
            ...
            while not done:
                ...
                action = net.activate(observation) # argmax because we have two output nodes, left or right
    ...
    def eval_genomes(genomes, config):
        for genome_id, genome in genomes:
            genome.fitness = MountainCar.eval_genome(genome, config)
    @staticmethod
    def run(config_file):
        ...
        sys.stdout = open('moutain-car-output.txt', 'w') 

        pe = neat.ParallelEvaluator(multiprocessing.cpu_count(), MountainCar.eval_genome)
